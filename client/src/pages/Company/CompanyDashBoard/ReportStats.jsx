...
const ReportStats = () => {
    ...
    const pieChartData = useMemo(
        () => [
            {
                name: "Good (8-10)",
                value: filteredData.reduce((acc, d) => acc + (d.good || 0), 0),
            },
            {
                name: "Average (5-7)",
                value: filteredData.reduce((acc, d) => acc + (d.average || 0), 0),
            },
            {
                name: "Poor (0-4)",
                value: filteredData.reduce((acc, d) => acc + (d.poor || 0), 0),
            },
        ],
        [filteredData]
    );
    ...
    const resetFilters = () =>
        setFilters({ range: 7, chartType: "bar", positionTitle: "" });

    return (
        <div className="container-fluid px-4">
            {/* Header */}
            <div className="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 className="h3 mb-0 fw-semibold text-primary">
                ...
                <div className="d-flex gap-2">
                    <CSVLink
                        data={csvData}
                        filename={`report-stats-${filters.range}-days.csv`}
                        className="btn shadow-sm fw-semibold text-white border-0 rounded-3 px-3 py-2"
                        style={{
                    ...
                    <button
                        ...
                        className="btn shadow-sm fw-semibold text-white border-0 rounded-3 px-3 py-2"
                        style={{
                            background: "linear-gradient(90deg, #f87171, #fbbf24)", // red → amber
            ...
            <div className="card border-0 shadow-sm mb-4 rounded-4">
                ...
                <div className="card-body">
                    <div className="row g-3">
                        <div className="col-md-3">
                            <label className="form-label small fw-bold">Time Range</label>
                            <select
                                name="range"
                                className="form-select form-select-sm"
                                onChange={handleFilterChange}
                        ...
                        <div className="col-md-3">
                            <label className="form-label small fw-bold">Chart Type</label>
                            <select
                                name="chartType"
                                className="form-select form-select-sm"
                                onChange={handleFilterChange}
                        ...
                        <div className="col-md-3 d-flex align-items-end">
                            <button
                                className="btn btn-outline-secondary w-100 rounded-3 fw-semibold shadow-sm"
                                onClick={resetFilters}
                            >
                                Reset Filters
                            </button>
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                {filters.chartType === "bar" ? (
                                    <BarChart data={filteredData}>
                                        ...
                                        <Tooltip content={<CustomTooltip />} />
                                        <Legend />
                                        <Bar dataKey="good" fill={COLORS[0]} name="Good (8-10)" />
                                        <Bar
                                            dataKey="average"
                                            fill={COLORS[1]}
                                            name="Average (5-7)"
                                        />
                                        <Bar dataKey="poor" fill={COLORS[2]} name="Poor (0-4)" />
                                    </BarChart>
                                ) : (
                                    <PieChart>
                                        <Pie
                ...
                <div className="col-lg-8 mb-4">
                    <div className="card border-0 shadow-sm h-100 rounded-4">
                        ...
                        >
                            ...
                            <small>By average interview score</small>
                        </div>
                        <div className="card-body">
                            {topJobTitles.length ? (
                                topJobTitles.map((item, idx) => (
                            ...
                            <div className="text-center mt-3">
                                {pieChartData.map((item, index) => (
                                    <div key={index} className="d-inline-block mx-2">
                                        <span className="fw-bold" style={{ color: COLORS[index] }}>
                                            {item.value}
                                        </span>
                                        <small className="d-block text-muted">{item.name}</small>
                                    </div>
                                ))}
                            </div>
                        </div>
