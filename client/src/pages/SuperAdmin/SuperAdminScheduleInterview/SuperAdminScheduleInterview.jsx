...
const SuperAdminScheduleInterview = () => {
    ...
    const { server, createNotification } = useAuthContext();
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isUpdate) {
                res = await axios.put(`${server}/api/v1/superadmin/updateInterviews/${formData._id}`,
                    interviewData,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
            ...
            createNotification(state?.applicant?._id, "INTERVIEW_SCHEDULED", `Your interview for the position of "${state?.job?.title}" has been scheduled.`)
            createNotification(state?.job?.companyId, "INTERVIEW_SCHEDULED", `An interview has been scheduled for the position of "${state?.job?.title}" with candidate ${state?.applicant?.fullname || 'a candidate'}.`)
            createNotification(formData?.interviewerId, "INTERVIEW_SCHEDULED", `You have been assigned to conduct an interview for the position of "${state?.job?.title}" at ${state?.job?.company}.`)
