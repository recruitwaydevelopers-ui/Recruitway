...
import SuperAdminSendMockInterviewLinkModal from './SuperAdminSendMockInterviewLinkModal';

const SuperAdminAllMockInterviews = () => {
    const navigate = useNavigate()
    const [activeTab, setActiveTab] = useState('upcoming');
    const [searchTerm, setSearchTerm] = useState('');
    const { isLoading, getAllMockInterviews, mockinterviewes, markAsCancelledForMock } = useSuperAdminContext();
    ...
    const sortedInterviews = mockinterviewes?.sort((a, b) => {
    ...
    // Counts for badges
    const upcomingCount = mockinterviewes?.filter(i => {
    ...
    const todayCount = mockinterviewes?.filter(i => {
    ...
    const overdueCount = mockinterviewes?.filter(i => {
        const interviewDateTime = new Date(i.start);
        return interviewDateTime < now && i.status !== 'completed' && i.status !== "cancelled";
    }).length || 0;

    const completedCount = mockinterviewes?.filter(i => i.status === 'completed').length || 0;

    const cancelledCount = mockinterviewes?.filter(i => i.status === 'cancelled').length || 0;

    const inprocessCount = mockinterviewes?.filter(i => i.status === 'inProcess').length || 0;
        ...
        <>
            <div className="container-fluid">
                <div className="container">
                    <h1 className="mb-4">Mock Interview Management</h1>
                                                ...
                                                <td>
                                                    <div className="d-flex flex-wrap gap-2">
                                                        {activeTab === 'upcoming' && (
                                                            interview.status === "cancelled" ? (
                                                                <button
                                                                    ...
                                                                    onClick={() => navigateToSchedulePage(interview?.requestId)}
                                                                >
                                                                ...
                                                                <>
                                                                    ...
                                                                    <button className="btn btn-sm btn-success" onClick={() => navigateToSchedulePage(interview?.requestId)}>
                                                                        Reschedule
                                                                    </button>
                                                                    {interview.status !== "cancelled" && (
                                                                        <button
                                                                            className="btn btn-sm btn-outline-danger px-3"
                                                                            onClick={() => markAsCancelledForMock(interview._id, 'declined')}
                                                                        >
                                                        ...
                                                        {activeTab === 'overdue' && (
                                                            <>
                                                                {interview.status !== "cancelled" && (
                                                                    <button className="btn btn-sm btn-danger" onClick={() => markAsCancelledForMock(interview._id, 'declined')}>
                                                                        Cancel
                                                                    </button>
                                                                )}
                                                                <button className="btn btn-sm btn-success" onClick={() => navigateToSchedulePage(interview?.requestId)}>
                                                        ...
                                                        {activeTab === 'completed' && (
                                                            <>
                                                                <button className="btn btn-sm btn-success" onClick={() => navigate("/superadmin/candidate-mock-report", { state: { id: interview.interviewId } })}>
                                                                    Get Report
                                                                </button>
                                                            </>
                                                        )}
                                        ...
                                        <div key={interview._id} className="card mb-3">
                                            <div className="card-body">
                                                ...
                                                <div className="d-flex flex-wrap gap-2">
                                                    {activeTab === 'upcoming' && (
                                                        interview.status === "cancelled" ? (
                                                            <button className="btn btn-sm btn-success flex-grow-1" onClick={() => navigateToSchedulePage(interview?.requestId)}>
                                                                Reschedule
                                                            </button>
                                                        ) : (
                                                            <>
                                                                ...
                                                                {interview.status !== "cancelled" && (
                                                                    <button className="btn btn-sm btn-danger flex-grow-1" onClick={() => markAsCancelledForMock(interview._id, 'declined')}>
                                                                        Cancel
                                                                    </button>
                                                                )}
                                                            </>
                                                    ...
                                                    {activeTab === 'completed' && (
                                                        <button className="btn btn-sm btn-success flex-grow-1" onClick={() => navigate("/superadmin/candidate-mock-report", { state: { id: interview.interviewId } })}>
                                                            Report
                                                        </button>
                                                    )}
                                                </div>
                    ...
                    {isModalOpen && selectedInterview && (
                        <SuperAdminSendMockInterviewLinkModal
                            isOpen={isModalOpen}
...
export default SuperAdminAllMockInterviews;