...
const getProfile = async (req, res) => {
    try {
        ...
        let pro = await CompanyProfile.findOne({ userId });
...
const updateJobPost = async (req, res) => {
    ...
    try {
        ...
        const jobDoc = await Job.findOne({ _id: applicationId, userId });
...
const getAllDoneInterviewsOfCompany = async (req, res) => {
    ...
    try {
        ...
        const [interviews, interviewsWithCV] = await Promise.all([
            Interview.find({ companyId, status: "completed" }).sort({ createdAt: -1 }),
            InterviewWithCV.find({ companyId, status: "completed" }).sort({ createdAt: -1 })
        ]);

        res.status(200).json({ success: true, data: { interviews, interviewsWithCV } });
    } catch (error) {
...
const getReportOfEachCandidate = async (req, res) => {
    ...
    try {
        // Step 1: find report
        const repo = await Report.findOne({ interviewId });
        ...
        if (repo.sentToCompany === false) {
            return res.status(403).json({ success: false, message: "Recruitway has not approved the report yet" });
        }
        ...
        const companyProfile = await CompanyProfile.findOne({ userId: repo.companyId });
        if (!companyProfile) {
            return res.status(404).json({ success: false, message: "Company profile not found." });
        }

        // Step 4: fetch interviewer profile
        const interviewerProfile = await InterviewerProfile.findOne({ userId: repo.interviewerId });
...
module.exports = {
    ...
    getAllDoneInterviewsOfCompany, getReportOfEachCandidate, scheduleInterview, getSuperAdmin
}
