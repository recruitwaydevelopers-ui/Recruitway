...
const SuperAdminProvider = ({ children }) => {
    ...
    const getAllMockInterviews = async () => {
        dispatch({ type: "SET_LOADING" });
        try {
            const response = await axios.get(`${server}/api/v1/mockRequest/get-all-mockinterviews-of-all-candidates`,
                {
                    headers:
                    {
                        Authorization: `Bearer ${token}`
            ...
            dispatch({ type: "SET_ALL_MOCK_INTERVIEWS_OF_ALL_CANDIDATES", payload: interviewes });
        } catch (error) {
            console.error("Error fetching companies:", error);
            dispatch({ type: "SET_LOADING_FALSE" });
            const errorMessage = error.response?.data?.message || "Something went wrong. Please try again.";
    ...
    const handleSendEmailForMock = async (interviewId, emailData) => {
        dispatch({ type: "SET_LOADING" });
        console.log(interviewId);
        console.log(emailData);

        try {
            const res = await axios.post(`${server}/api/v1/mockRequest/candidateandinterviewer/${interviewId}/send-invite/for-mock`,
                emailData,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
    ...
    const markAsCancelledForMock = async (interviewId, status) => {
        const res = await axios.put(`${server}/api/v1/mockRequest/updateStatus/${interviewId}`,
            { status },
            {
                headers: { Authorization: `Bearer ${token}` },
            }
    ...
    const getReportOfMockInterview = async (id) => {
        dispatch({ type: "SET_LOADING" });
        try {
            const response = await axios.get(`${server}/api/v1/interviews/getMockReport/${id}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
            ...
            const { mockReport } = response.data;
            dispatch({ type: "SET_MOCK_REPORT_OF_CANDIDATE", payload: mockReport });
        } catch (error) {
            dispatch({ type: "SET_LOADING_FALSE" });
            toast.dismiss()
            toast.error(err.response?.data?.message || 'Failed to fetch candidates');
    ...
    const getDetailedReportOfInterview = async (id) => {
        dispatch({ type: "SET_LOADING" });
        try {
            const response = await axios.get(`${server}/api/v1/interviews/getDetailedReport/${id}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
            ...
            const { detailedReport, aiReport } = response.data;
            dispatch({ type: "SET_DETAILED_REPORT_OF_CANDIDATE", payload: { detailedReport, aiReport } });
        } catch (error) {
            dispatch({ type: "SET_LOADING_FALSE" });
            toast.dismiss()
            toast.error(err.response?.data?.message || 'Failed to fetch candidates');
    ...
    return (
        <SuperAdmminContext.Provider value={{
            ...
            handleSendEmailForMock,
            markAsCancelled,
            markAsCancelledForCV,
            markAsCancelledForMock,
