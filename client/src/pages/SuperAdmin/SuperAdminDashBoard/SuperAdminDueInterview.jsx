...
const SuperAdminDueInterview = ({ interviewsList }) => {
    const interviews = useMemo(() => {
        const now = new Date();
        return interviewsList
            .filter((intv) => {
                const startTime = new Date(intv.start);
                const haspasses = startTime < now;
                const hasValidStatus = intv.status === "scheduled";

                return haspasses && hasValidStatus;
            })
    ...
    return (
        <div className="card shadow h-100">
            <div className="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-primary text-white">
                <div>
                    <h6 className="m-0 font-weight-bold text-light">Due Interviews</h6>
                    <small className="text-light opacity-75">Scheduled but not started interviews</small>
                </div>
            ...
            <div className="card-footer bg-white text-muted small">
                <div className="d-flex justify-content-between align-items-center">
                    <span>Showing next {interviews.length} due interviews</span>
...
export default SuperAdminDueInterview;